...
/**
 * A trait to wrap user authentication credentials.
 */
sealed trait Credential

case class PasswordCredential(password: String) extends Credential
case class TwAuthCredential(token: AccessToken) extends Credential
...
class Security[T] {
   ...
   */
  def authenticate[U <: Credential](username: String, credential: U): T = {
...
object Security {
  ...
  def authenticate[U <: Credential](username: String, credential: U) = {
    val token = realm.authenticate(username, credential)
